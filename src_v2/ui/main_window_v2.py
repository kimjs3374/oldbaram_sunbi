...
# 2026-04-25 v2 main_window: 본체 v1 그대로 (4000줄). worker만 v2 facade.
# 그 외 UI 컴포넌트 (region_picker / overlay / status_strip / dialogs)는
# src/ui/ 그대로 재사용 — src 무수정 원칙. src_v2/ui/에는 필요 없는 컴포넌트
# 복사 안 함 (절대 경로 import로 src/ui/* 직접 사용).
from src.utils.logger_setup import _setup_logger
from src.utils.win_helpers import _user32, _is_fg_hwnd, frame_to_qpix
...
# worker만 v2 stack — facade가 v1 인터페이스 100% 노출
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, cfg, initial_role: str = "healer"):
        super().__init__()
        self.cfg = cfg
        self.worker = None           # HealerWorker or AttackerWorker
        # 2026-04-27 동작 변경: 진입점은 default 만 제공.
        # _load_settings 의 디스크 role 우선 (사용자 라디오 마지막 선택 보존).
        # 디스크 없을 때만 entry default 적용. forced 강제 폐기 — 사용자가 라디오로
        # 격수↔도사 토글하면 그 선택이 다음 실행에서도 유지됨.
        # (_on_role_change 가 반대 역할 워커 stop 책임 — auto-start 버그 차단됨.)
        _ir = str(initial_role or "healer").lower()
        self.role = "attacker" if _ir == "attacker" else "healer"
        self._entry_default_role: str = self.role
        ...
        _pre_role = self.role
        self._load_settings()
        # 디스크 role 우선. 디스크 없거나 _load_settings 가 role 변경 안 했으면
        # entry default (_entry_default_role) 가 그대로 self.role.
        # 라디오는 self.role 따라 강제 sync (불일치 시 신호 발화 → _on_role_change
        # 가 워커/heartbeat 정리).
        try:
            if self.role == "attacker" and not self.rb_attacker.isChecked():
                self.rb_attacker.setChecked(True)
            elif self.role == "healer" and not self.rb_healer.isChecked():
                self.rb_healer.setChecked(True)
        except Exception:
            pass
        self._append_log(
            f"[ROLE] entry_default={self._entry_default_role} pre_load={_pre_role} "
            f"final={self.role}"
        )
        self._apply_role_ui()
    ...
    def _build_ui(self):
        ...
        # __init__(initial_role) 강제값에 라디오 동기. signal 차단 후 set →
        # _on_role_change 가 워커 재기동을 일으키지 않도록.
        self.rb_healer.blockSignals(True)
        self.rb_attacker.blockSignals(True)
        if self.role == "attacker":
            self.rb_attacker.setChecked(True)
        else:
            self.rb_healer.setChecked(True)
        self.rb_healer.blockSignals(False)
        self.rb_attacker.blockSignals(False)
        ...
        from src.ui.dialogs import SkillRangeDialog
    ...
    def _on_role_change(self, _checked=False):
        ...
        # 역할 전환 시 반대 역할 워커가 살아있으면 즉시 정지.
        # (이전 버그: listener/heartbeat 만 정리하고 워커는 살아남아
        #  반대 역할 OCR 영역에서 healer cd OCR 계속 돌고 [CD-OCR-MISS]
        #  raw_lines 에 화면 위 무관 텍스트가 잡혀 들어감.)
        try:
            w = getattr(self, "worker", None)
            if w is not None:
                wcls = type(w).__name__
                wrong = (
                    (self.role == "attacker" and "Healer" in wcls) or
                    (self.role == "healer" and "Attacker" in wcls)
                )
                if wrong:
                    self._append_log(f"[ROLE] 반대 역할 워커 정지 ({wcls})")
                    try:
                        if hasattr(w, "stop"):
                            w.stop()
                        if hasattr(w, "wait"):
        ...
        if self.role != "attacker":
            ...
            if worker_log is not None:
                try:
                    ...
                    pass
            return
        # 2026-04-27 worker 가 AttackerWorker 일 때만 위임. HealerWorker 인
        # 경우 send_control 가 no-op 리턴 → 송신 0. 클래스 이름 검사로 분기.
        # 어느 경우든 healer worker 면 fallback 으로 직접 socket 송신.
        _wcls = type(self.worker).__name__ if self.worker is not None else ""
        _is_atk_worker = bool(self.worker is not None
                              and "Attacker" in _wcls
                              and hasattr(self.worker, "send_control"))
        if _is_atk_worker:
            ok = self.worker.send_control(target_idx, cmd)
            self._append_log(f"[CTRL] target={tag} cmd={cmd} ok={ok} (worker={_wcls})")
            return
        # Fallback — worker 미시작. ControlCmd 직접 unicast.
        try:
            import socket as _socket
            from src.net.protocol import ControlCmd, now_ms  # type: ignore
            peers = list(getattr(self.cfg.net, "peers", []) or [])
            try:
                pe = self.peers_edit.text().strip()
                pe_list = [p.strip() for p in pe.split(",") if p.strip()]
                if pe_list:
                    peers = pe_list
            except Exception:
                pass
            ...
            self._append_log(
                f"[CTRL] target={tag} cmd={cmd} ok={ok_any} "
                f"(no-worker-fallback peers={peers}:{port})"
            )
        except Exception as _e:
            self._append_log(f"[CTRL] fallback 송신 예외: {_e}")
        ...
        if src_ip:
            ...
            if src_ip not in peers:
                ...
                try:
                    ...
                    pass
            # P1-1 (v1_gap_fix_list): resolved_row_idx 우선. payload contract.
            # peers 매칭 실패면 -1 → 자동 등록 결과 idx 사용.
            r_idx = int(d.get("resolved_row_idx", -1))
            idx = r_idx if r_idx >= 0 else peers.index(src_ip)
        else:
            # src_ip 없음 (구형 송신자/테스트). reported_idx 사용 + 경고.
            idx = int(d.get("reported_idx", d.get("src_idx", 0)))
            if not getattr(self, "_no_ip_warn_emitted", False):
                self._no_ip_warn_emitted = True
                self._append_log(
                    "[CD-RECV-WARN] src_ip 비어있음 — reported_idx fallback. "
                    "다중 힐러 환경에서 row 충돌 위험."
                )
    ...
    def _on_test_xp_ocr(self) -> None:
        ...
        try:
            from src.capture.screen import Grabber
        ...
        try:
            from src.vision.xp_ocr import XpOcr
            ocr = XpOcr()
            ocr.set_region(*xp_r)
    ...
    def _on_test_hpmp(self) -> None:
        ...
        try:
            ...
            return
        try:
            from src.vision.hpmp import HpMpReader
    ...
    def _on_test_block_a(self) -> None:
        ...
        try:
            if combined:
                from src.input.target_sequence import run_block_ab_combined
                ...
                from src.input.target_sequence import run_block_a_test
    ...
    def _on_test_block_b(self) -> None:
        ...
        try:
            ...
            cycler = None
        try:
            from src.input.target_sequence import run_block_b_test
    ...
    def _setup_global_hotkeys(self) -> None:
        ...
        """
        from src.input.global_hotkeys import (
    ...
    def _reinject_own_skill_names(self) -> None:
        ...
        try:
            if (self.worker is None
                ...
                return
            from src.ui.hunter_helper_panel import get_rank_skills
            names = [
                nm for (nm, _cd)
    ...
    def _tick_overlay_visibility(self) -> None:
        ...
        try:
            ...
            from src.input.keys import find_windows_by_process
            from src.capture.screen import user32 as _u32
            tw = str(getattr(self.cfg.input, "target_window", "msw.exe"))
            wins = find_windows_by_process(tw) if tw.lower().endswith(".exe") else []
            if not wins:
                self._set_overlays_visible(False)
    ...
    def _tick_msw_tracker(self) -> None:
        ...
        try:
            from src.input.keys import find_windows_by_process
            from src.capture.screen import get_window_rect
            tw = str(getattr(self.cfg.input, "target_window", "msw.exe"))
            wins = find_windows_by_process(tw) if tw.lower().endswith(".exe") else []
            if not wins:
                return
    ...
    def _on_numlock_off(self):
        try:
            from src.input.numlock_cycle import ensure_numlock_off
    ...
    def start_worker(self):
        # 2026-04-28 fix: UI freeze 중 중복 클릭 → 두 번째 워커 인스턴스 →
        # 같은 UDP 포트 bind 시도 → WinError 10048 → 격수좌표 영원히 못 받음.
        # 사용자 신고: "실행 눌렀는데 반응 없으니 또 눌렀다."
        # root: _start_X 가 메인 thread 에서 무거운 init (YOLO load 등) 수행 →
        # UI freeze → 클릭 큐에 누적. setEnabled(False) + starting flag 둘 다.
        if getattr(self, "_starting", False):
            self._append_log("[START-GUARD] 이미 시작 중 — 중복 클릭 무시")
            return
        if self.worker and self.worker.isRunning():
            self._append_log("[START-GUARD] 워커 이미 실행 중 — 무시")
            return
        self._starting = True
        # 즉시 disable — 큐에 누적된 추가 클릭 자체를 차단.
        try:
        ...
        try:
            ...
            # 시작 실패 → 버튼 복구 (다시 누를 수 있게).
            self._append_log(f"[START-FAIL] {_e!r}")
            try:
    ...
    def _on_attacker_seen(self, ip: str, port: int) -> None:
        """ControlListener가 첫 패킷 src_addr을 알려주면 Heartbeat 와 워커에 전달.

        2026-04-27: 워커의 uplink_sender 도 갱신 (v1 동적 격수 IP 학습 동치).
        이전엔 heartbeat 만 갱신 → CooldownUplink 의 sender 가 cfg.peers (힐러
        들 IP) 로 잘못 송신 → 격수 측 힐러 HP/MP/cd 영원 빈 칸.
        """
        if self._heartbeat is not None:
            self._heartbeat.set_attacker_addr(str(ip), int(port))
        # 워커 facade 의 uplink_sender 에도 격수 IP 전달.
        try:
            if (self.worker is not None
                    and hasattr(self.worker, "set_attacker_addr")):
                self.worker.set_attacker_addr(str(ip), int(port))
        except Exception:
            pass
        self._append_log(f"[CONNECT] 격수 감지 {ip}:{port} → heartbeat + uplink 송신")
    ...
    def _stop_ctrl_listener(self) -> None:
        if self._ctrl_listener is None:
            return
        # 2026-04-28 fix v2: socket.shutdown(SHUT_RDWR) → recv loop 즉시 깨우기 +
        # OS 가 socket 즉시 release. close 만으로는 Windows 가 grace period 둠 →
        # 다음 워커 bind 가 30회 retry 안에 못 뚫음 (WinError 10048).
        # shutdown 박으면 OS 가 즉시 포트 free → 워커 bind 한 번에 성공.
        import socket as _sk
        _sock = getattr(self._ctrl_listener, "_sock", None)
        # 1) shutdown 먼저 — recv blocking 깨움 + OS 알림.
        ...
        # 4) OS port release 확인 wait — Windows UDP 는 보통 즉시 release 지만
        # shutdown 후에도 짧은 grace 가 남는 케이스 방어. 100ms.
        try:
            import time as _t
            _t.sleep(0.1)
        except Exception:
    ...
    def _handle_remote_cmd_active(self, cmd: str, target_idx: int) -> None:
        """워커 활성 상태에서 격수가 보낸 ControlCmd 처리.

        2026-04-27 신규: 이전엔 워커 활성 시 ControlCmd 받아도 무시 →
        격수 UI 의 stop/pause/follow 버튼 작동 안 함.
        """
        c = str(cmd or "").lower()
        if c == "ping":
            return
        self._append_log(
            f"[REMOTE-ACTIVE] cmd={c} target={target_idx}"
        )
        if c == "stop":
            try:
                self.stop_worker()
            except Exception as _e:
                self._append_log(f"[REMOTE-ACTIVE] stop 예외: {_e}")
            return
        if c in ("start", "pause"):
            armed_on = (c == "start")
            try:
                ...
                self.chk_arm.blockSignals(False)
                # facade 의 armed setter 직접 호출 (signal 차단 상태라 자동 안 됨).
                if self.worker is not None:
                    self.worker.armed = bool(armed_on)
            except Exception as _e:
                self._append_log(f"[REMOTE-ACTIVE] arm 토글 예외: {_e}")
            return
        if c in ("follow_on", "follow_off"):
            ...
            try:
                ...
                if self.worker is not None:
                    self.worker.follow_only = bool(on)
            except Exception as _e:
                self._append_log(f"[REMOTE-ACTIVE] follow 토글 예외: {_e}")
    ...
    def _start_healer(self):
        # 2026-04-27 BUG-FIX: peers/port 를 cfg 에 반영 후 worker 생성.
        # 이전엔 누락 — facade._build_adapters 가 빈 cfg.net.peers 로 호출되어
        # uplink_sender 미생성 → CooldownUplink 미시작 → 힐러→격수 UDP 송신 0
        # → 격수 UI 의 힐러 행 (HP/MP/cd) 영원 빈 칸. (attacker 쪽 _start_attacker
        # 는 같은 갱신을 line 3704 에서 이미 수행 — 비대칭 누락이었음.)
        try:
            peers_raw = self.peers_edit.text().strip()
            peers = [p.strip() for p in peers_raw.split(",") if p.strip()]
            if peers:
            ...
            try:
                ...
                pass
            self._append_log(
                f"[v2] healer cfg sync peers={list(self.cfg.net.peers)} "
                f"port={int(getattr(self.cfg.net, 'port', 0) or 0)} "
                f"recv_port={int(getattr(self.cfg.net, 'attacker_recv_port', 45455) or 45455)}"
            )
        except Exception as _e:
            self._append_log(f"[v2] healer peers/port 반영 실패: {_e}")
        ...
        # 2026-04-27 워커 활성 시 격수 ControlCmd 처리 (정지/일시정지 등).
        try:
            self.worker.cmd_received.connect(self._handle_remote_cmd_active)
        except Exception:
            pass
        self.worker.start()
    ...
    def stop_worker(self):
        # 진단: 정지 시 role/worker class 명시 — 재시작 후 격수 좌표 안 받는 케이스 추적용.
        try:
            wcls = type(self.worker).__name__ if self.worker else "None"
            self._append_log(
                f"[STOP] role={self.role} worker={wcls} → stop 요청"
            )
        except Exception:
            pass
        if self.worker:
    ...
    def _collect_settings(self) -> dict:
        ...
        from src.ui.settings_io import collect
        return collect(self)

    def _save_settings(self):
        """JSON 저장. 본문은 ui.settings_io.save 로 분리."""
        from src.ui.settings_io import save
        save(self)

    def _load_settings(self):
        """JSON 로드 + UI 복원. 본문은 ui.settings_io.load 로 분리."""
        from src.ui.settings_io import load
        load(self)
