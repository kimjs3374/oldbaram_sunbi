"""좌표 숫자(0~9) 전처리 정규화(_normalize) + (레거시) 템플릿 매칭.

현재 운영 인식은 digit_cnn(onnx)이며 이 모듈은 그 전처리(_normalize: 종횡비
보존 정사각 패딩)를 제공한다. match()/add()/save()/load() 는 학습데이터 부트
스트랩용 레거시. torch/paddle/easyocr 의존 0.
